...
"%TEMP%\LxRunOffline.exe" "i" "-n" "%DISTRO%" "-f" "%TEMP%\Ubuntu2%UBUVER%04.tar.gz" "-d" "%DISTROFULL%"
...
REM Convert imported distro to WSL2 before package installation
ECHO [%TIME:~0,8%] Converting distro to WSL2
IF EXIST "%PROGRAMFILES%\WSL\WSL.EXE" (
  "%PROGRAMFILES%\WSL\WSL.EXE" --set-version "%DISTRO%" 2
) ELSE (
