...
%GO% "mkdir -p /usr/share/fonts/truetype ; cp /mnt/c/Windows/Fonts/*.ttf /usr/share/fonts/truetype 2>/dev/null || true ; ssh-keygen -A ; id -u xrdp >/dev/null 2>&1 || useradd -r -M -d /var/run/xrdp -s /usr/sbin/nologin xrdp ; getent group ssl-cert >/dev/null 2>&1 || addgroup --system ssl-cert ; adduser xrdp ssl-cert >/dev/null 2>&1 || true ; if [ ! -f /etc/ssl/certs/ssl-cert-snakeoil.pem ] || [ ! -f /etc/ssl/private/ssl-cert-snakeoil.key ]; then make-ssl-cert generate-default-snakeoil --force-overwrite >/dev/null 2>&1 || true ; fi" > NUL
